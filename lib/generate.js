...
module.exports = {
    dir: function(destDir, outputDir) {
        ...
        var indexList = '';
        //编译md
        $.recurse(pptDir, function(abspath, rootdir, subdir, filename) {

            var html = parser(abspath, template['markdown.tpl']);
            if (html) {
        ...
        indexList = grunt.template.process(template['list.tpl'] || fs.readFileSync(staticDir + 'list.tpl', 'utf-8').toString(), {
            data: data
        });
