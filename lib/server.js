  ...
  let app = connect(
    ...
    function (req, res) {
      ...
      if (url === '/') {
        ...
        return
      } else if (dirname.indexOf('/md') === 0) {
        // md文件解析
        url = URL.parse(req.url).pathname
        let relDir = dirname.slice(4).replace(/\//g,path.sep)
        relDir += relDir ? path.sep : '';
        ...
        realPath = pptDir + relDir + basename
        debug('markdown', realPath, basename)
        markdown(realPath, dirname.slice(3) + '/' + basename, res, argvObj, queryObj)
        return
...
function markdown (realPath, url, res, argvObj, queryObj) {
  if (fs.existsSync(realPath)) {
    ...
    try {
      let html = mdParser(content, path.dirname(url), function () {}, argvObj, queryObj)
      // 添加socket的html代码
      let extraHtml = ''
      if (argvObj.watch) {
...
function pptlist (res, dir, argvObj) {
  ...
  $.recurse(dir, function (realpath, rootdir, subdir, filename) {
    ...
    let url = '/md/' + subdir.replace(new RegExp(path.sep,'g'),'/') + (subdir ? '/':'') + filename
    console.log(url);console.log(subdir);
    ...
    return /\.(md|markdown)$/.test(filename) || $.isDir(path.join(dir, filename),true)
  })
