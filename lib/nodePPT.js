...
var ppt = module.exports = {
    ...
    start: function(args) {
        ...
        if (fs.existsSync(path.join(cwd, 'config.json'))) {
            ...
            defaultConfig = require(rootDir + 'config.json');
        ...
        if (dir === '') {
            //优选本路径下config
            if (config.pptDir && $.isDir(path.join(cwd, config.pptDir)), true) {
                dir = path.join(cwd, config.pptDir);
                isSubPath = true;
            } else {
        ...
        if ($.isDir(dir)) {
            require(libDir + '/server').start(port, dir, isSubPath, config);
        }
    ...
    generate: function(args) {
        ...
        if (filename !== '') {
            require(libDir + '/generate').output(filename);
        } else if ($.isDir(curRoot)) {
            require(libDir + '/generate').dir(curRoot, output);
        }
    }
};
