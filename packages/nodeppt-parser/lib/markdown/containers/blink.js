...
module.exports = {
  ...
  render(tokens, idx) {
    ...
    if (token.nesting === 1) {
      ...
      if (clsIndex >= 0) {
        attrs[clsIndex][1] += cmIndex >= 0 ? ` flexblock blink border ${attrs[cmIndex][1]}` : ` flexblock blink border`
      } else {
        attrs.push(['class', cmIndex >= 0 ? `flexblock blink border ${attrs[cmIndex][1]}` : `flexblock blink border`])
      }
